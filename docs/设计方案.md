...
```text
1. 接收知识采集模块输出的 JSON / JSONL；
2. 根据 doc_type 和 chunk_type 选择分块策略；
3. 生成 normalized_text；
4. 添加 task_scope、device_type、project_id、source 信息；
5. 计算 chunk_hash，执行去重；
6. 调用 embedding 模型生成向量；
7. 写入 Chroma；
8. 写入 SQLite chunk 元数据表；
9. 返回向量化报告。
```
...
2. 文件元数据写入 SQLite；
3. 解析结果写入 SQLite；
4. chunk 内容和 metadata 写入 SQLite；
5. embedding 写入 Chroma；
6. 生成结果和校验结果写入 SQLite；
7. 周期性执行本地备份；
8. 支持按 project_id 导出完整知识库包。
```
...
```text
1. 接收 task_type、user_query、project_id、当前画布上下文；
2. 进行 Query 改写和实体提取；
3. 按任务类型选择 collection 和过滤条件；
4. 执行语义召回、关键词召回、规则召回；
5. 合并候选结果；
6. 根据任务规则和相似度进行重排；
7. 生成 Top-K 知识包；
8. 输出检索解释、候选素材、候选变量、相似案例。
```
...
```text
1. 接收 task_type、user_query、retrieval_result、project_context；
2. 选择对应 Prompt 模板；
3. 注入 Schema 摘要、枚举字典和硬规则；
4. 注入当前画布、控件、变量、素材等上下文；
5. 对长文本知识进行摘要压缩；
6. 生成最终任务 Prompt；
7. 记录 Prompt 版本、输入 token 估算和上下文来源；
8. 发送给 Agent 生成模块。
```
...
4. 执行枚举字典校验；
5. 根据任务类型执行布局 / 绑点 / Patch / 脚本规则校验；
6. 生成错误列表；
7. 对确定性错误执行规则修复；
8. 对语义性错误调用 RepairAgent；
9. 再次校验；
10. 输出合格结果或失败报告。
```
